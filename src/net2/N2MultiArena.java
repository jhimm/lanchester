...
public class N2MultiArena {
    ...
    public ArrayList<N2ForceUnit> forces = new ArrayList();
    public boolean isBattle = true;
    public double timeStep = 0.1;
    public HashMap<N2ForceUnit, Integer> forceMap = new HashMap();
    ...
    public void addForce(N2ForceUnit f) {
        int num = forces.size();
        forces.add(f);
        forceMap.put(f, num);
        f.setForceIndex(num);
    ...
    public ArrayList<N2ForceUnit> getForces() {
        return this.forces;
    }
...
//                    numGone++;
    ...
    public boolean checkAboveFloors() {
        ...
        for (int i1 = 0; i1 < forces.size(); i1++) {
            if (stillAlive[i1]) {
                N2ForceUnit f1 = forces.get(i1);
                for (int i2 = 0; i2 < forces.size(); i2++) {
                    if (stillAlive[i2]) {
                        N2ForceUnit f2 = forces.get(i2);
...
//                    setStances(f1, f2);
                    setStances(i1, i2);
                    double a1 = f2.getForceOffenseMultiplier()/f1.getForceDefenseMultiplier();
                    double b2 = f1.getForceOffenseMultiplier()/f2.getForceDefenseMultiplier();
                    if (isBattle) {
    ...
    public void setStances(int i1, int i2) {
        ...
        double ratio1to2 = (currentF2 > 1.e-1 ? currentF1 / currentF2 : 10000.);
...
//                    currentStance = AthenaConstants.WITHDRAW_POSTURE;
                    System.out.println(i1 + " Changed from Attack to Withdraw vs " + i2 + " at time " + currentTime);
                } else {
                    stanceArray[i1][i2] = AthenaConstants.DEFEND_POSTURE;
//                    currentStance = AthenaConstants.DEFEND_POSTURE;
                    System.out.println(i1 + " Changed from Attack to Defend vs " + i2 + " at time " + currentTime);
                }
            }
...
//                currentStance = AthenaConstants.ATTACK_POSTURE;
                System.out.println(i1 + " Changed from Defend to Attack vs " + i2 + " at time " + currentTime);
            } else if (ratio1to2 < f1.dwFloorMap.get(f2)) {
                stanceArray[i1][i2] = AthenaConstants.WITHDRAW_POSTURE;
//                currentStance = AthenaConstants.WITHDRAW_POSTURE;
                System.out.println(i1 + " Changed from Defend to Withdraw vs " + i2 + " at time " + currentTime);
            }
        } else if (currentStance == AthenaConstants.WITHDRAW_POSTURE) {
            if (ratio1to2 > f1.adCeilingMap.get(f2)) {
                stanceArray[i1][i2] = AthenaConstants.ATTACK_POSTURE;
//                currentStance = AthenaConstants.ATTACK_POSTURE;
                System.out.println(i1 + " Changed from Withdraw to Attack vs " + i2 + " at time " + currentTime);
            } else if (ratio1to2 > f1.dwCeilingMap.get(f2)) {
                stanceArray[i1][i2] = AthenaConstants.DEFEND_POSTURE;
//                currentStance = AthenaConstants.DEFEND_POSTURE;
                System.out.println(i1 + " Changed from Withdraw to Defend vs " + i2 + " at time " + currentTime);
            }
        }
...
//                    System.out.println(i2 + " Changed from Attack to Withdraw vs " + i1+" at time "+currentTime);
//                } else {
//                    stanceArray[i2][i1] = AthenaConstants.DEFEND_POSTURE;
//                    System.out.println(i2 + " Changed from Attack to Defend vs " + i1+" at time "+currentTime);
//                }
//            }
//        } else if (currentStance == AthenaConstants.DEFEND_POSTURE) {
//            if (ratio2to1 > f2.dwCeilingMap.get(f1)) {
//                stanceArray[i2][i1] = AthenaConstants.ATTACK_POSTURE;
//                System.out.println(i2 + " Changed from Defend to Attack vs " + i1+" at time "+currentTime);
//            }
//            if (ratio2to1 < f2.dwFloorMap.get(f1)) {
//                stanceArray[i2][i1] = AthenaConstants.WITHDRAW_POSTURE;
//                System.out.println(i2 + " Changed from Defend to Withdraw vs " + i1+" at time "+currentTime);
//            }
//        } else if (currentStance == AthenaConstants.WITHDRAW_POSTURE) {
//            if (ratio2to1 > f2.adCeilingMap.get(f1)) {
//                stanceArray[i2][i1] = AthenaConstants.ATTACK_POSTURE;
//                System.out.println(i2 + " Changed from Withdraw to Attack vs " + i1+" at time "+currentTime);
//            } else if (ratio2to1 > f2.dwCeilingMap.get(f1)) {
//                stanceArray[i2][i1] = AthenaConstants.DEFEND_POSTURE;
//                System.out.println(i2 + " Changed from Withdraw to Defend vs " + i1+" at time "+currentTime);
//            }
//        }
    }

    public int initializeStance(N2ForceUnit f1, N2ForceUnit f2) {
        if(f1.isFriend(f2)){
            return AthenaConstants.ALLIED_POSTURE;
        }
        double currentF1 = f1.getNumber();
        double currentF2 = f2.getNumber();
...
//        return findStance(f1, ratio1to2);
    ...
    public void setFloor(int i, int j) {
        N2ForceUnit f1 = forces.get(i);
        N2ForceUnit f2 = forces.get(j);
...
//            currentCeiling[i][j] = 2 * currentFloor[i][j];
    ...
    public double[] getInitialNumbers(ArrayList<N2ForceUnit> forces) {
        double[] nums = new double[forces.size()];
        for (int i1 = 0; i1 < forces.size(); i1++) {
            nums[i1] = forces.get(i1).getNumber();
            System.out.println("Force "+i1+" = "+nums[i1]);
        }
        return nums;
    }
}
